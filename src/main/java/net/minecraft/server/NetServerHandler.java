...
import org.bukkit.event.player.PlayerMoneyAskEvent;
...
public class NetServerHandler extends NetHandler implements ICommandListener {
    ...
    @Override
    public void a(Packet133Money packet){
    	float money = packet.money;
        PlayerMoneyAskEvent event = new PlayerMoneyAskEvent(packet.money, this.getPlayer());
        this.server.getPluginManager().callEvent(event);
        if(money != event.getMoney())
        {
           packet.money = event.getMoney();
           sendPacket(packet);
        }
    }
}
